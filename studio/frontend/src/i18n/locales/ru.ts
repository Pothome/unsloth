  ...
  settings: {
    ...
    resources: {
      ...
      modelMemory: {
        ...
        noRamReserveHint: "Не резервирует полную копию весов в ОЗУ. Сохраняет загрузку через отображение файла в память в llama.cpp и убирает --no-mmap и --mlock. Само отображение остаётся, поэтому затронутые загрузчиком страницы продолжают учитываться за этим процессом, пока система их не освободит.",
